...
class VisionService:
    ...
    def take_screenshot(self) -> Optional[str]:
        """Captures the primary screen using mss, resizes it, and returns a base64 JPEG."""
        try:
            import io
            ...
            try:
                ...
                # Fallback to PIL ImageGrab if mss is not available in the runtime environment
                from PIL import ImageGrab
                img = ImageGrab.grab(all_screens=False)
                
            # Resize if it's too large to save bandwidth (max 1920x1080)
            ...
            logger.error("[VISION] Failed to capture screen: %s", e)
            return None
