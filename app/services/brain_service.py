...
-> 'open_app (app name)' — Open a local desktop application.
   "Open Notepad" → open_app Notepad
   "Launch Calculator" → open_app Calculator

-> 'close_app (app name)' — Close a local desktop application.
   "Close Notepad" → close_app Notepad
   "Kill Calculator" → close_app Calculator

-> 'system_control (command)' — Shutdown, restart, or sleep the computer.
   "Shutdown my computer" → system_control shutdown
   "Restart PC" → system_control restart
   "Put laptop to sleep" → system_control sleep
...
class BrainService:
    ...
    _TASK_FEW_SHOTS = [
        ...
        ("open python idle", "open_app python idle"),
        ("launch notepad", "open_app notepad"),
        ("close word", "close_app word"),
        ("kill calculator", "close_app calculator"),
        ("what is on my screen", "screen_vision what is on my screen"),
        ("look at my screen and tell me what this is", "screen_vision tell me what this is"),
        ("read the text on my screen", "screen_vision read the text on my screen"),
        ("read the pdf document.pdf", "read_file document.pdf"),
        ("summarize the file presentation.pdf from downloads", "read_file presentation.pdf"),
        ("write a python script to calculate pi and run it", "run_code write a script to calculate pi"),
        ("run this python code to sort an array", "run_code python code to sort an array"),
        ("what am i doing today", "calendar what am i doing today"),
        ("schedule a meeting with sarah for tomorrow at 2pm", "calendar schedule a meeting with sarah for tomorrow at 2pm"),
        ("list files in my downloads", "list_dir Downloads"),
        ("what is in this folder", "list_dir ."),
    ]
        ...
        if decisions:
            for task_type, clean_query in decisions:
                ...
                if task_type == "open":
                    ...
                    payload["app_name"] = clean_query or user_message
                elif task_type == "close_app":
                    payload["app_name"] = clean_query or user_message
                elif task_type == "screen_vision":
                    payload["prompt"] = clean_query or user_message        
                elif task_type == "read_file":
                    payload["file_name"] = clean_query or user_message        
                elif task_type == "run_code":
                    payload["prompt"] = clean_query or user_message        
                elif task_type == "calendar":
                    payload["prompt"] = clean_query or user_message
                elif task_type == "system_control":
                    payload["command"] = clean_query or "sleep"
                elif task_type == "list_dir":
                    payload["path"] = clean_query.strip() or "."
                elif task_type == "execute_skill":
                    payload["skill_id"] = clean_query
                elif task_type == "start_record":
                    if " FROM " in clean_query:
                        parts = clean_query.split(" FROM ")
                        payload["skill_name"] = parts[0].strip()
                        payload["video_file"] = parts[1].strip() if parts[1].strip() != "None" else None
                    else:
                        payload["skill_name"] = clean_query.strip()
...
Classify. Output EXACTLY ONE category name."""
    ...
    def _parse_task_decisions(self, raw_response: str) -> List[Tuple[str, str]]:
        ...
        TASK_PREFIXES = [
            ...
            "open_webcam", "close_webcam", "screen_vision", "read_file", "run_code", "calendar",
            "open_app", "close_app", "system_control", "email", "whatsapp",
    ...
    def _rule_based_task(self, msg: str) -> List[str]:
        ...
        if m.startswith(("shutdown", "restart", "sleep")):
            tasks.append("system_control")
            
        if any(x in m for x in ["what is on my screen", "look at my screen", "read my screen", "what's on my screen", "my screen"]):
            tasks.append("screen_vision")
            
        if "read" in m and any(x in m for x in ["pdf", "file", "document", "csv", "txt"]):
            tasks.append("read_file")
            
        if "run" in m and any(x in m for x in ["python", "script", "code"]):
            tasks.append("run_code")
            
        if any(x in m for x in ["calendar", "schedule", "meeting", "events", "agenda", "what am i doing"]):
            tasks.append("calendar")
            
        if m.startswith(("close ", "kill ", "quit ", "exit ", "stop ")) and not any(x in m for x in ["facebook", "youtube", "google", "netflix", "gmail", "instagram", "twitter", "linkedin", "website", "http", ".com", "webcam", "camera"]):
            tasks.append("close_app")
            
        elif m.startswith(("open ", "launch ", "start ")) and not any(x in m for x in ["facebook", "youtube", "google", "netflix", "gmail", "instagram", "twitter", "linkedin", "website", "http", ".com"]):
            tasks.append("open_app")
        elif m.startswith(("open ", "launch ", "go to ", "visit ", "can you open ")) or \
